...
dir = reactive({ file.dir( input$country , dir.base = input$data.directory  )  })
...
ous.file = reactive({
  ...
  dir = file.dir( input$country , dir.base = input$data.directory )
...
selectInput("filter_split", label = "Filter output:" , 
              choices = NULL , 
              selected = NULL  ) 
...
observeEvent(
  ...
  if ( !input$split %in% 'None' ){
    ...
    updateSelectInput( session, 'filter_split' , choices =  c( 'All', splits ) )
    
    print( paste( splits , collapse = ',' ) )
  } else {
    
    updateSelectInput( session, 'filter_split' , choices =  c( 'All' ) )
  }
} )
...
trendData = reactive({
  ...
  if (input$selected & !is.null( selectedOUs() ) ){
    ...
    if ( !input$split %in% 'None' & !input$filter_split %in% 'All' ){
        print( 'filter_split is not null' )
        .d = .d %>% filter( .data[[ input$split ]] %in% input$filter_split )
      }
