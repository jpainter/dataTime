...
inputPanel(
  ...
  checkboxInput( "merge", label ='If total, merge datasets first', value = FALSE ) ,
...
data.folder = reactive({
  ...
  file.dir( country = input$country , dir.base = input$data.directory )
})
...
formulas =  reactive({
  ...
  if ( !file.exists( formula.file() ) ) return( NULL )
...
dataset = reactive({
  ...
  # testing:
  tic()
  if ( !file.exists( rds_data_file() )) return( NULL )
...
d = reactive({
 ...
 req( input$level )
  ...
  print( 'd: max Month ' ); print( max( data$Month ))
...
selectedOUs <- reactive({
  ...
  if ( input$mostReports ){
    ...
    if ( !input$count.any & !input$total  ){
     ...
     mr = data %>% as_tibble %>%
       filter( year( Month ) >= as.integer( input$startingYear ) ) %>% distinct( Month, orgUnit ) %>%
       group_by( orgUnit ) %>%
       summarise( n = n() ) %>%
       arrange( desc( n ))
...
### Number of Facilties Reporting by Month (plot_reporting_by_month)
...
plotData = reactive({
   req( d() )
    ...
    print( 'nrow( plotData )')  ; print( nrow( data )) 
    print( 'names( plotData )')  ; print( names( data )) 
  return( data )
})
...
data.total = reactive({
  ...
  req( group_by_cols() )
    ...
    print( 'data.total group_by_cols:'); print( group_by_cols() )
    
  if ( length( group_by_cols() ) < 2  ){
    
       key.cols = NULL 
  } else {
       key.cols =  rlang::sym( setdiff( group_by_cols() , 'Month' ) )
  }
  ...
  # This version combines all categories because the categories may be different 
  # between datasets. If possible, would be best to compare categories and 
  # combine by category if possible.  
  # At present, fails if data not totaled because cant filter input$data
  if ( input$merge & input$total ){
    

    dataset_group_by_cols  = rlang::syms( 
      unique( c( group_by_cols() , 'orgUnit' , 'dataSet' ) )
    )

    combined_group_by_cols  = rlang::syms( c( group_by_cols() , 'orgUnit'  )
    )
    ...
    data = 
      ...
      group_by(  !!! combined_group_by_cols  ) %>%
      summarise( dataCol = mean( dataCol , na.rm = TRUE  ) ) %>%
    ...
    data.total = 
      ...
      as_tsibble( index = Month, key =  all_of(key.cols) ) %>%
      fill_gaps( .full = TRUE  ) %>%
      mutate( facilities ="All" ,
              total = replace_na( total , 0) )  # for plotting, replace missing with zero 
    # print( 'glimpse( data.total )') ; print( glimpse( data.total ))
    print( 'nrow( data.total )')  ; print( nrow( data.total )) 
    print( 'names( data.total )')  ; print( names( data.total )) 
    return( data.total )
...
data.selected = reactive({
  ...
  if ( input$merge & input$total ){
    ...
    dataset_group_by_cols  = rlang::syms( c( group_by_cols() , 
                                             'orgUnit' , 'dataSet' )
    )
    ...
    data.selected =   data %>% 
      ...
      group_by(  across( all_of( group_by_cols() )) ) %>%
      summarise( total = sum( dataCol , na.rm = TRUE  ) ) %>%
      as_tsibble( index = Month, key =  key.cols ) %>%
      fill_gaps( .full = TRUE  ) %>%
...
trendData = reactive({
  ...
  if (input$selected & !is.null( selectedOUs() ) ){
    ...
    if ( !input$split %in% 'None' & !input$filter_data %in% 'All' ){
        ...
        .d = .d %>% filter( .data[[ input$split ]] %in% input$filter_data )
      }
...
plotTrends = reactive({
  ...
  if ( input$split != 'None' ){
      ...
      .d = trendData() %>% as_tibble %>% 
          filter( .data[[ input$split ]] %in% input$filter_display )
  ...
  if (input$evaluation) g = g + 
    geom_line( data = tsForecast(), aes( y = .mean ) ,   
               color = 'blue' , alpha = .5 ) +
    geom_vline( xintercept = as.Date(yearmonth( input$evaluation_month  )) , 
  ...
  if (input$smooth) g = g + 
    geom_smooth( data = trendData() %>% as_tibble , alpha = .75 )
