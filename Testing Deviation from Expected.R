...
if ( run_previous ){ 
  ...
  ous = unique( preEvent$orgUnit )
  n = length( ous )
  p <- progress::progress_bar$new( total = n)
  tic()
  
    pre.ts_model = future_map( 1:n , 
  ...
  pre.model = data.table::rbindlist( map( pre.ts_model, 2 ) ) %>%
    ...
    nest( data = c(.model, Month, value, .fitted, pct.change , .resid, .innov)) %>%
   ...
   unnest( data ) %>%
    ...
    summarise( 
       ...
       values = sum( !is.na( value ) ) ,
        ...
        mean_value = mean( value , na.rm = TRUE ) ,
        median_value = mean( value , na.rm = TRUE ) ,
        sd = sd( value , na.rm = TRUE )
        )
...
if ( run_previous ){ 
  ...
  expected = readRDS(paste0( dir, expected.file ) ) 
...
actual_expected = expected %>%
    ...
    mutate( 
      .fitted = ifelse( .median < 0, 0 , .median ) ,
      pct.change = ( ( actual + 1 ) / ( .fitted  + 1 ) ) -1
      ) %>% 
    as_tsibble( key = c(orgUnit, var), index = Month ) 
...
post.event.summary.total %>% autoplot( mean.pct.change ) + 
...
.title = NULL
.subtitle =  paste0('Key: Line is mean deviation form expected.', 
...
g = 
  ...
  autoplot( mean.pct.change , color = 'dark red',
            size = 1.5 , alpha = .9 ) +
