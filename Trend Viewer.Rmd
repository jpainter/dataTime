...
```{css my-css, echo = FALSE, eval = FALSE }
# .chart-title {
#     font-size: 48px;
# }
.tab-content {
...
formula.file = reactive({ 
  ...
  ff = paste0( data.folder() , 
          files( search = 'Formula' , dir = data.folder() )  %>% 
            most_recent_file() 
          )
...
observeEvent( input$all_categories == TRUE , {
...
observeEvent( input$all_categories == FALSE , {
  print( 'updating data_categories' )
  updateSelectInput( session, 'data_categories' , 
                     choices =   unique( dataset()$data )  ) 
} )
...
```{r}
  ...
  checkboxInput( "all_categories", label = 'All categories', value = FALSE ) 
# )
  ...
  # splitLayout( cellWidths = c("75%", "25%") , 
    ...
    div(id = "expr-container",
      ...
      selectInput("data_categories", label = "DataElement/Category" , 
                choices = NULL  ,
                selected = 1 ,
                width = "100%" ,
                multiple = TRUE ,
 ...
 # splitLayout( cellWidths = c("75%", "25%") , 
    ...
    checkboxInput( "dataset_merge", label ='Total unmerged datasets', value = TRUE ) 
...
plotData = reactive({
   ...
   if ( length( selectedOUs()) > 0 ) data  = data %>%
      mutate( Selected = ifelse( 
        orgUnit %in% selectedOUs() , 'Reporting Each Month', 'Inconsistent Reporting' )
      )
...
checkboxInput('components', label = 'Visualize model components' ,
            value = FALSE )
...
# Difference over each iteration, which is naturally weighted
mpe_distribution = function( forecast_distribution , var ){
    
  cat(' mpe_distribution:' )
    var = enquo( var )
