...
dates = reactive({ 
    ...
    unique( data.total() %>% pull( !! rlang::sym( .period )) )  
    })
...
#                               choices =  dates()  , 
#                               selected = ifelse( period() %in% 'Month' ,
#                                                  max(  dates()  ) - month(12) ,
#                                      max(  dates()  ) - week(52)            )
#                               ) 
#   } )
...
key.mape = reactive({
  ...
  e = truth %>%
    ...
    summarise( 
          ...
          actual = nth( actual , mid_point ) 
          ) %>%
  as_tibble() %>%
      mutate( !! input$agg_level := 
                as.character( !! rlang::sym( input$agg_level  ) ) )
  
  print( "mape"); #glimpse(e )
  return( e )
})
...
plotTrends = reactive({
  ...
  if ( input$evaluation ){
    ...
    eval_date = as.Date( 
      yearmonth( input$evaluation_month  ) 
      )
   ...
   g = g + 
    ...
    #            aes( x = Month, y = .mean ) ,
    #            color = 'blue' , alpha = .5 ) 
    geom_vline( xintercept = eval_date ,
                color = 'black', alpha = 1 ) +
    annotate( "text" ,
              x = eval_date ,
              y = Inf ,
    ...
    geom_label_repel( data =  key.mpe()
               ...
               label = percent( mpe , accuracy = 1.0 ) )
               )
  }
  ...
  if ( input$pre_evaluation ){
    ...
    print( 'eval_date is' ); print( eval_date )
    g = g + 
    ...
    geom_label_repel( data =  key.mape()
               , aes( x = !! rlang::sym( period() )  ,  
               label = percent( mape, accuracy = 1.0 ) )
               , y = Inf
               )
